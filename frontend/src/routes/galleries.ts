...
router.delete('/:id', async (req, res) => {
  ...
  try {
    // Zuerst alle zugehörigen Bilder abrufen
    const { rows: images } = await db.query(
      'SELECT filename FROM images WHERE gallery_id = $1',
      [id]
    );
    ...
    // Galerie aus der Datenbank löschen (CASCADE wird die Bilder automatisch löschen)
